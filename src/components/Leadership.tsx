        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-5xl mx-auto">
          {leaders.map((leader, index) => (
            <motion.div
              ...
              className="group"
            >
              <div className="relative overflow-hidden rounded-2xl border border-border bg-card/50 backdrop-blur-sm hover:border-accent/30 transition-all duration-500">
                {/* Photo Area */}
                <div className="relative h-64 bg-gradient-to-br from-navy-light to-navy overflow-hidden">
                  {/* Decorative pattern */}
                  <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_120%,hsl(var(--accent)/0.15),transparent_70%)]" />
                  
                  {/* Avatar with initials */}
                  <div className="absolute inset-0 flex items-center justify-center">
                    <motion.div
                      whileHover={{ scale: 1.05 }}
                      className="w-32 h-32 rounded-full bg-gold-gradient flex items-center justify-center shadow-2xl"
                    >
                      <span className="font-display text-4xl font-bold text-navy-dark">
                        {leader.initials}
                      </span>
                    </motion.div>
                  </div>
